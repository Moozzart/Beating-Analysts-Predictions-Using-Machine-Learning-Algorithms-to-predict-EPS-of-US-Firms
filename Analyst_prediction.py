...
X = modelling_data[['ACTUALt-4','shockt-1','ACTUALt-1','MEDEST','MEANEST','Index','ACTUAL']]
y = modelling_data[['ACTUAL']]

X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.33, random_state=42)

rf = RandomForestRegressor(n_estimators=80)
rf.fit(X_train.drop(['MEDEST','MEANEST','Index','ACTUAL'],axis=1),y_train)
pred = rf.predict(X_test.drop(['MEDEST','MEANEST','Index','ACTUAL'],axis=1))
...
print(X_test[['MEANEST','MEANEST','Prediction']].corr())
l = ['Prediction','MEDEST','MEANEST']
for i in l:
    X_test['diff'+str(i)] = abs((X_test[i] - X_test['ACTUAL']))

print(('Percentage Mean Analyst Prediction Beaten:', X_test['diffPrediction']<X_test['diffMEANEST']).mean())
print('MERGING DATA FRAME WITH EXACT ANALYST ESTIMATES DATA FRAME')
...
X = data_with_common_index[['ACTUALt-4','shockt-1','ACTUALt-1','MEDEST','MEANEST','Index','ACTUAL']]
...
print((X_test['diffPrediction']<X_test['diffMEANEST']).mean())
X_test_2=X_test
l = ['Prediction','MEDEST','MEANEST']
for i in l:
    X_test_2['diff'+str(i)] = abs((X_test_2[i] - X_test_2['ACTUAL']))
print('Percentage Mean Analyst Prediction Beaten:',(X_test_2['diffPrediction']<X_test_2['diffMEANEST']).mean())

diff_pred = X_test_2['diffPrediction'].values
Index_list = X_test_2['Index'].values
...
for i in range(len(Index_list)):
    ...
    diff_.append((temp['analyst_diff']>=X_test_2.loc[Index_list[i]]['diffPrediction']).mean())
